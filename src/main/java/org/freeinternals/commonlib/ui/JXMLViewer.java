...
public class JXMLViewer extends JPanel {
	...
	public JXMLViewer(InputStream xml) {
		...
		try {
			XMLTreeTable treeTable = new XMLTreeTable(new XMLTreeTableModel(
					new InputSource(xml)));
			this.tabbedPane.addTab("XML View", new JScrollPaneTreeTable(
					treeTable));
		} catch (Exception ex) {
			// Logger.getLogger(JXMLViewer.class.getName()).log(Level.SEVERE,
			// null, ex);
			...
			JLabel label = new JLabel(
					"Exception occured when parsing the XML data.");
