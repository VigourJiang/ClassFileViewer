...
public class JBinaryViewer extends JPanel {
	...
	public JBinaryViewer() {
		...
		panelLayout.putConstraint(SpringLayout.WEST, this.rowViewer, 2,
				SpringLayout.WEST, panel);
		panelLayout.putConstraint(SpringLayout.NORTH, this.rowViewer, 2,
				SpringLayout.NORTH, panel);
		panelLayout.putConstraint(SpringLayout.SOUTH, this.rowViewer, -4,
				SpringLayout.SOUTH, panel);
		panelLayout.putConstraint(SpringLayout.EAST, this.rowViewer,
				JRowViewer.WIDTH_VALUE, SpringLayout.WEST, panel);

		left = 2 + JRowViewer.WIDTH_VALUE + 2;
		right = left + JRawDataViewer.WIDTH_VALUE;
		panelLayout.putConstraint(SpringLayout.WEST, this.rawViewer, left,
				SpringLayout.WEST, panel);
		panelLayout.putConstraint(SpringLayout.NORTH, this.rawViewer, 2,
				SpringLayout.NORTH, panel);
		panelLayout.putConstraint(SpringLayout.SOUTH, this.rawViewer, -4,
				SpringLayout.SOUTH, panel);
		panelLayout.putConstraint(SpringLayout.EAST, this.rawViewer, right,
				SpringLayout.WEST, panel);

		left = right + 2;
		right = left + JAsciiDataViewer.WIDTH_VALUE;
		panelLayout.putConstraint(SpringLayout.WEST, this.asciiViewer, left,
				SpringLayout.WEST, panel);
		panelLayout.putConstraint(SpringLayout.NORTH, this.asciiViewer, 2,
				SpringLayout.NORTH, panel);
		panelLayout.putConstraint(SpringLayout.SOUTH, this.asciiViewer, -4,
				SpringLayout.SOUTH, panel);
		panelLayout.putConstraint(SpringLayout.EAST, this.asciiViewer, right,
				SpringLayout.WEST, panel);
	...
	private int getRowMax() {
		return (this.data != null) ? this.getRowCount(this.data.length) : 0;
	}
	...
	private void updateViewContent() {
		...
		if ((extent + this.vBar.getValue()) > (this.rowMax + JBinaryViewer.ROW_EMPTYROW_COUNT)) {
			int diff = (this.rowMax + JBinaryViewer.ROW_EMPTYROW_COUNT)
					- extent;
			diff = (diff > 0) ? diff : 0;
	...
	private void updateSelection() {
		final int startPos = this.selectedStartIndex - this.vBar.getValue()
				* JBinaryViewer.ROW_ITEM_MAX;
		final int lengtgMax = this.getExtent() * JBinaryViewer.ROW_ITEM_MAX;
		int length;

		if (startPos > 0) {
			length = Math.min(this.selectedLength, lengtgMax); // Improve
																// Performance
	 ...
	 * 
	 * @param startPos
	 *            the start position of the byte(s) to be visible
	 */
	...
	class MouseWheelAdapter implements MouseWheelListener {
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			...
			case MouseWheelEvent.WHEEL_UNIT_SCROLL:
				JBinaryViewer.this.vBar.setValue(JBinaryViewer.this.vBar
	...
	class KeyboardAdapter implements KeyListener {
		...
		public void keyPressed(KeyEvent e) {
			...
			case KeyEvent.VK_PAGE_UP:
				JBinaryViewer.this.vBar.setValue(JBinaryViewer.this.vBar
						.getValue()
						- JBinaryViewer.this.vBar.getVisibleAmount());
				break;
