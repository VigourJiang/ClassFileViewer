...
public class PosDataInputStream extends DataInputStream implements DataInputEx {
	 ...
	 * <code>startPos</code> of original stream, with length <code>length</code>
	 * .
	 * 
	 * @return A partial {@link PosDataInputStream} object
	 */
	public PosDataInputStream getPartialStream(int startPos, int length) {
		return new PosDataInputStream(new PosByteArrayInputStream(this.getBuf(
	 ...
	 *         buffer, or <code>-1</code> if there is internal error, the input
	 *         stream is not <code>PosByteArrayInputStream</code>.
	 */
	public int getPos() {
		int pos = -1;
	 ...
	 * 
	 * @param startPos
	 *            Start Position of the original byte array
	 * @param length
	...
	public String readASCII(int length) throws IOException {
		if (length <= 0) {
			throw new IllegalArgumentException(String.format(
					"Parameter length should be greater than 0. (length = %d)",
					length));
		}
	...
	public int backward(int i) {
		...
		if (this.in instanceof PosByteArrayInputStream) {
			...
			((PosByteArrayInputStream) this.in)
					.setPos(result = ((currentPos - i) > 0) ? (currentPos - i)
							: 0);
		}
