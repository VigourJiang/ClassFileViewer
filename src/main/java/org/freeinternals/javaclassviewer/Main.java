...
public final class Main extends JFrame {
	...
	private void createMenu() {
		...
		menuItem_FileOpen.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O,
		...
		menuItem_FileExit.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X,
	...
	private void menu_FileOpen() {
		final FileNameExtensionFilter filterClass = new FileNameExtensionFilter(
				"Class File", "class");
		final FileNameExtensionFilter filterJar = new FileNameExtensionFilter(
				"Jar File", "jar");
			...
			if (file.getName().endsWith(".jar")) {
				...
				JOptionPane
						.showMessageDialog(
								this,
								String.format(
										"Un-supported file type. Please select a '.jar' or '.class' file. \nFile: %s",
										file.getPath()), this.getTitle(),
	...
	private void open_JarFile(final File file) {
		try {
			this.zftree = new JTreeZipFile(new JarFile(file, false,
					JarFile.OPEN_READ));
			this.zftree.addMouseListener(new MouseAdapter() {
				...
				public void mousePressed(final MouseEvent e) {
					...
					zftree_DoubleClick(zftree.getPathForLocation(e.getX(),
							e.getY()));
				}
			});
		} catch (IOException ex) {
			Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
			JOptionPane.showMessageDialog(this, ex.toString(), this.getTitle(),
	...
	private void menu_HelpHomepage() {
		try {
			Desktop.getDesktop().browse(
					new URI("https://code.google.com/p/freeinternals/"));
		} catch (Exception ex) {
	...
	private void zftree_DoubleClick(final TreePath tp) {
		final DefaultMutableTreeNode node = (DefaultMutableTreeNode) this.zftree
		...
		if (ze == null) {
			JOptionPane.showMessageDialog(this,
					"Node Object [zip entry] is emtpy.", this.getTitle(),
					JOptionPane.WARNING_MESSAGE);
		} else {
	...
	private void showClassWindow(final ZipEntry ze) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				new JFrameClassFile(sbTitle.toString(), b, Main.this)
						.setVisible(true);
			}
		});
	}
