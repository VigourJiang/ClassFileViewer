...
public class JTreeClassFile extends JTree {
	...
	private void generateTreeNodes() {
		try {
			this.root = new DefaultMutableTreeNode(new JTreeNodeClassComponent(
					0, this.classFile.getByteArraySize(), "Class File"));
			...
			Logger.getLogger(JTreeClassFile.class.getName()).log(Level.SEVERE,
					null, itne);
		}
	}
	...
	private void generateTreeNodeClsssFileVersion()
		...
		final DefaultMutableTreeNode minor_version = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getMinorVersion()
						.getStartPos(), this.classFile.getMinorVersion()
						.getLength(), "minor_version: "
						+ this.classFile.getMinorVersion().getValue()));
		this.root.add(minor_version);

		final DefaultMutableTreeNode major_version = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getMajorVersion()
						.getStartPos(), this.classFile.getMajorVersion()
						.getLength(), "major_version: "
						+ this.classFile.getMajorVersion().getValue()));
	...
	private void generateConstantPool() throws InvalidTreeNodeException {
		...
		final DefaultMutableTreeNode constant_pool_count = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getCPCount()
						.getStartPos(),
		...
		final DefaultMutableTreeNode constant_pool = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(STARTPOS_constant_pool,
						cp[cpCount - 1].getStartPos()
								+ cp[cpCount - 1].getLength()
								- STARTPOS_constant_pool, "constant_pool"));
		...
		for (int i = 1; i < cpCount; i++) {
			if (cp[i] != null) {
				cp_info = new DefaultMutableTreeNode(
						new JTreeNodeClassComponent(cp[i].getStartPos(),
								cp[i].getLength(), i + ". " + cp[i].getName()));
	...
	private void generateClassDeclaration() throws InvalidTreeNodeException {
		...
		final DefaultMutableTreeNode access_flags = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getAccessFlags()
						.getStartPos(), this.classFile.getAccessFlags()
						.getLength(), "access_flags: "
						+ this.classFile.getAccessFlags().getModifiers()));
		this.root.add(access_flags);

		sb.append("this_class: ");
		sb.append(this.classFile.getThisClass().getValue());
		sb.append(String.format(" - [%s]", this.classFile
				.getCPDescription(this.classFile.getThisClass().getValue())));
		final DefaultMutableTreeNode this_class = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getThisClass()
						.getStartPos(), this.classFile.getThisClass()
		...
		sb.append(String.format(" - [%s]", this.classFile
				.getCPDescription(this.classFile.getSuperClass().getValue())));
		final DefaultMutableTreeNode super_class = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getSuperClass()
						.getStartPos(), this.classFile.getSuperClass()
						.getLength(), sb.toString()));
		this.root.add(super_class);

		final int interfaceCount = this.classFile.getInterfacesCount()
				.getValue();
		final DefaultMutableTreeNode interfaces_count = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getInterfacesCount()
						.getStartPos(), this.classFile.getInterfacesCount()
						.getLength(), "interfaces_count: " + interfaceCount));
		...
		if (interfaceCount > 0) {
			...
			final DefaultMutableTreeNode interfacesNode = new DefaultMutableTreeNode(
					new JTreeNodeClassComponent(interfaces[0].getStartPos(),
							interfaces[interfaceCount - 1].getStartPos()
									+ interfaces[interfaceCount - 1]
											.getLength()
									- interfaces[0].getStartPos(), "interfaces"));
	...
	private void generateFields() throws InvalidTreeNodeException {
		...
		final DefaultMutableTreeNode fields_count = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getFieldCount()
						.getStartPos(), this.classFile.getFieldCount()
						.getLength(), "fields_count: " + fieldCount));
		...
		if (fieldCount > 0) {
			...
			final DefaultMutableTreeNode fieldsNode = new DefaultMutableTreeNode(
					new JTreeNodeClassComponent(fields[0].getStartPos(),
							fields[fieldCount - 1].getStartPos()
									+ fields[fieldCount - 1].getLength()
									- fields[0].getStartPos(), "fields"));
			this.root.add(fieldsNode);

			DefaultMutableTreeNode fieldNode;
			for (int i = 0; i < fieldCount; i++) {
				fieldNode = new DefaultMutableTreeNode(
						new JTreeNodeClassComponent(fields[i].getStartPos(),
								fields[i].getLength(), String.format(
										"field %d: %s", i + 1,
	...
	private void generateMethods() throws InvalidTreeNodeException {
		...
		final DefaultMutableTreeNode methods_count = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getMethodCount()
						.getStartPos(), this.classFile.getMethodCount()
						.getLength(), "methods_count: " + methodCount));
		...
		if (methodCount > 0) {
			...
			final DefaultMutableTreeNode methodsNode = new DefaultMutableTreeNode(
					new JTreeNodeClassComponent(methods[0].getStartPos(),
							methods[methodCount - 1].getStartPos()
									+ methods[methodCount - 1].getLength()
									- methods[0].getStartPos(), "methods"));
			this.root.add(methodsNode);

			DefaultMutableTreeNode methodNode;
			for (int i = 0; i < methodCount; i++) {
				methodNode = new DefaultMutableTreeNode(
						new JTreeNodeClassComponent(methods[i].getStartPos(),
								methods[i].getLength(), String.format(
										"method %d: %s", i + 1,
	...
	private void generateAttributes() throws InvalidTreeNodeException {
		...
		final DefaultMutableTreeNode attrs_count = new DefaultMutableTreeNode(
				new JTreeNodeClassComponent(this.classFile.getAttributeCount()
						.getStartPos(), this.classFile.getAttributeCount()
						.getLength(), "attributes_count: " + attrCount));
		...
		if (attrCount > 0) {
			...
			final DefaultMutableTreeNode attrsNode = new DefaultMutableTreeNode(
					new JTreeNodeClassComponent(attrs[0].getStartPos(),
							attrs[attrCount - 1].getStartPos()
									+ attrs[attrCount - 1].getLength()
									- attrs[0].getStartPos(), "attributes"));
			this.root.add(attrsNode);

			DefaultMutableTreeNode attrNode;
			for (int i = 0; i < attrCount; i++) {
				attrNode = new DefaultMutableTreeNode(
						new JTreeNodeClassComponent(attrs[i].getStartPos(),
								attrs[i].getLength(), i + ". "
										+ attrs[i].getName()));
