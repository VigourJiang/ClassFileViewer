...
class JTreeNodeClassComponent {
	...
	public JTreeNodeClassComponent(final int startPos, final int length,
			final String text) throws InvalidTreeNodeException {
		if (startPos < 0) {
			throw new InvalidTreeNodeException(
					"Start position cannot be less than zero; it is '"
							+ startPos + "'.");
		}

		if (length < 0) {
			throw new InvalidTreeNodeException(
					"Length cannot be less than zero; it is '" + length + "'.");
		}
