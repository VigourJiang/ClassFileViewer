...
public class JSplitPaneClassFile extends JSplitPane {
	...
	public JSplitPaneClassFile(final byte[] byteArray, JFrame top) {
		try {
			...
			Logger.getLogger(JSplitPaneClassFile.class.getName()).log(
...
//		byte[] jsonBytes = mapper.writeValueAsBytes(classFile);
	...
	private void jTreeClassFileSelectionChanged(final TreeSelectionEvent evt) {
		...
		if (obj instanceof DefaultMutableTreeNode) {
			...
			if (obj instanceof JTreeNodeClassComponent) {
				...
				this.binaryViewer.setSelection(objTncc.getStartPos(),
						objTncc.getLength());
				...
				if (objTncc.getText().equals("code")) {
					...
					final byte[] data = this.classFile.getClassByteArray(
	...
	private void generateOpcodeParseResult(byte[] opcodeData) {
		...
		// The Extracted Code
		sb.append("<pre>");
		sb.append(org.freeinternals.commonlib.util.Tool.getByteDataHexView(opcodeData, true));
		sb.append("\n");
		...
		if (!codeResult.CPIndexes.isEmpty()) {
			...
			for (Integer i : codeResult.CPIndexes) {
				sb.append(String.format("<li>%s</li>", HTMLKit
						.EscapeFilter(this.classFile.getCPDescription(i))));
			}
			sb.append("</ol>");
		}
	...
	private void generateClassReport() {
		...
		if (count > 0) {
			...
			this.generateCPTypeReport(sb, CPInfoList, count,
					AbstractCPInfo.CONSTANT_Class);
			sb.append("Constant Pool - Field");
			this.generateCPTypeReport(sb, CPInfoList, count,
					AbstractCPInfo.CONSTANT_Fieldref);
			sb.append("Constant Pool - Method");
			this.generateCPTypeReport(sb, CPInfoList, count,
					AbstractCPInfo.CONSTANT_Methodref);
		...
		if (count > 0) {
			...
			for (FieldInfo field : fields) {
				sb.append(String.format("<li>%s</li>",
						HTMLKit.EscapeFilter(field.getDeclaration())));
			}
			sb.append("</ol>");
		}
		sb.append(HTMLKit.NewLine());
		...
		if (count > 0) {
			...
			for (MethodInfo method : methods) {
				sb.append(String.format("<li>%s</li>",
						HTMLKit.EscapeFilter(method.getDeclaration())));
			}
			sb.append("</ol>");
		}
		sb.append(HTMLKit.NewLine());
	...
	private void generateCPTypeReport(StringBuilder sb,
			AbstractCPInfo[] CPInfoList, int count, short tag) {
		sb.append(HTMLKit.NewLine());
		sb.append("<ul>");
		for (int i = 1; i < count; i++) {
			if (CPInfoList[i].getTag() == tag) {
				sb.append(String.format("<li>%d. %s</li>", i, HTMLKit
						.EscapeFilter(this.classFile.getCPDescription(i))));
			}
		}
		sb.append("</ul>");
