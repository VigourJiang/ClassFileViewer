...
public class JTreeZipFile extends JTree {
	...
	private DefaultMutableTreeNode addTreeNode(
			final DefaultMutableTreeNode parent, final String childText,
			final boolean isFolder, final ZipEntry ze) {
		if (parent == null) {
			throw new IllegalArgumentException("parent node cannot be null.");
		}
		if (childText == null || childText.isEmpty()) {
			throw new IllegalArgumentException(
					"child node text cannot be null or empty.");
		}

		DefaultMutableTreeNode child = null;

		if (parent.isLeaf()) {
			child = new DefaultMutableTreeNode(new JTreeNodeZipFile(childText,
					ze));
			parent.add(child);
		} else {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (childText.equals(childTreeNode.toString())
						&& childTreeNode instanceof DefaultMutableTreeNode) {
			...
			if (child == null) {
				...
				child = new DefaultMutableTreeNode(new JTreeNodeZipFile(
						childText, ze));
				parent.add(child);
			}
		}
