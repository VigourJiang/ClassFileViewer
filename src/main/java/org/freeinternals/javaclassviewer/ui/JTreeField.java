...
final class JTreeField {
	...
	public static void generateTreeNode(final DefaultMutableTreeNode rootNode,
			final FieldInfo field_info, final ClassFile classFile)
			throws InvalidTreeNodeException {
		if (field_info == null) {
			return;
		}
		...
		rootNode.add(new DefaultMutableTreeNode(new JTreeNodeClassComponent(
				startPos, 2, "access_flags: " + field_info.getAccessFlags()
						+ ", " + field_info.getModifiers())));
		rootNode.add(new DefaultMutableTreeNode(new JTreeNodeClassComponent(
				startPos + 2, 2, "name_index: " + field_info.getNameIndex())));
		rootNode.add(new DefaultMutableTreeNode(new JTreeNodeClassComponent(
				startPos + 4, 2, "descriptor_index: "
						+ field_info.getDescriptorIndex())));
		rootNode.add(new DefaultMutableTreeNode(new JTreeNodeClassComponent(
				startPos + 6, 2, "attributes_count: " + attributesCount)));

		if (attributesCount > 0) {
			final AttributeInfo lastAttr = field_info
