...
public class AttributeExceptions extends AttributeInfo {
	...
	AttributeExceptions(final u2 nameIndex, final String type,
			final PosDataInputStream posDataInputStream) throws IOException,
			FileFormatException {
		super(nameIndex, type, posDataInputStream);

		this.number_of_exceptions = new u2();
		this.number_of_exceptions.value = posDataInputStream
				.readUnsignedShort();
		...
		if (excpCount > 0) {
			...
			for (int i = 0; i < excpCount; i++) {
				...
				this.exception_index_table[i].value = posDataInputStream
						.readUnsignedShort();
			}
		}
