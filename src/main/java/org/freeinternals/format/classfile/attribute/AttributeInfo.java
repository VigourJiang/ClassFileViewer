 ...
 * @since JDK 6.0
 * @see <a
 *      href="http://www.freeinternals.org/mirror/java.sun.com/vmspec.2nded/ClassFile.doc.html#43817">
...
public class AttributeInfo extends ClassComponent {
	 ...
	 * 
	 * @see <a
	 *      href="http://www.freeinternals.org/mirror/java.sun.com/vmspec.2nded/ClassFile.doc.html#1405">
	 *      VM Spec: The ConstantValue Attribute </a>
	 */
	 ...
	 * 
	 * @see <a
	 *      href="http://www.freeinternals.org/mirror/java.sun.com/vmspec.2nded/ClassFile.doc.html#79996">
	 *      VM Spec: The InnerClasses Attribute </a>
	 */
	 ...
	 * 
	 * @see <a
	 *      href="http://www.freeinternals.org/mirror/java.sun.com/vmspec.2nded/ClassFile.doc.html#80128">
	 *      VM Spec: The Synthetic Attribute </a>
	 */
	 ...
	 * 
	 * @see <a
	 *      href="http://www.freeinternals.org/mirror/java.sun.com/vmspec.2nded/ClassFile.doc.html#22856">
	 *      VM Spec: The LineNumberTable Attribute </a>
	 */
	...
	public static final String TypeLocalVariableTypeTable = "LocalVariableTypeTable";
	/**
	 * The name for {@code Deprecated} attribute type.
	 * 
	 * @see <a
	 *      href="http://www.freeinternals.org/mirror/java.sun.com/vmspec.2nded/ClassFile.doc.html#78232">
	 *      VM Spec: The Deprecated Attribute </a>
	 */
	public static final String TypeDeprecated = "Deprecated";

	public static final String TypeSignature = "Signature";
	public static final String TypeRuntimeVisibleAnnotations = "RuntimeVisibleAnnotations";
	public static final String TypeRuntimeInvisibleAnnotations = "RuntimeInvisibleAnnotations";
	...
	AttributeInfo(final u2 nameIndex, final String type,
	...
	public static AttributeInfo parse(
			...
			final AbstractCPInfo[] cp) throws IOException, FileFormatException {
		AttributeInfo attr = new AttributeInfo();

		final u2 attrNameIndex = new u2();
		attrNameIndex.value = posDataInputStream.readUnsignedShort();
		if (AbstractCPInfo.CONSTANT_Utf8 == cp[attrNameIndex.value].getTag()) {
			final String type = ((ConstantUtf8Info) cp[attrNameIndex.value])
					.getValue();
			if (TypeConstantValue.equals(type)) {
				attr = new AttributeConstantValue(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeCode.equals(type)) {
				attr = new AttributeCode(attrNameIndex, type,
						posDataInputStream, cp);
			} else if (TypeExceptions.equals(type)) {
				attr = new AttributeExceptions(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeInnerClasses.equals(type)) {
				attr = new AttributeInnerClasses(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeSynthetic.equals(type)) {
				attr = new AttributeSynthetic(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeSourceFile.equals(type)) {
				attr = new AttributeSourceFile(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeLineNumberTable.equals(type)) {
				attr = new AttributeLineNumberTable(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeLocalVariableTable.equals(type)) {
				attr = new AttributeLocalVariableTable(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeLocalVariableTypeTable.equals(type)) {
				attr = new AttributeLocalVariableTypeTable(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeDeprecated.equals(type)) {
				attr = new AttributeDeprecated(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeSignature.equals(type)) {
				attr = new AttributeSignature(attrNameIndex, type,
						posDataInputStream);
			} else if (TypeRuntimeInvisibleAnnotations.equals(type)
					|| TypeRuntimeVisibleAnnotations.equals(type)) {
				attr = new AttributeRuntimeAnnotations(attrNameIndex, type,
						posDataInputStream);
			} else {
				attr = new AttributeExtended(attrNameIndex, Extended + type,
						posDataInputStream);
			}
		} else {
			throw new FileFormatException(
					String.format(
							"Attribute name_index is not CONSTANT_Utf8. Constant index = %d, type = %d.",
							attrNameIndex.value,
							cp[attrNameIndex.value].getTag()));
		}
	...
	protected void checkSize(final int endPos) throws FileFormatException {
		if (this.startPos + this.length != endPos) {
			throw new FileFormatException(
					String.format(
							"Attribute analysis failed. type='%s', startPos=%d, length=%d, endPos=%d",
							this.getName(), this.startPos, this.length, endPos));
		}
	}
