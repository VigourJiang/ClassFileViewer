 ...
 *      VM Spec: The Java Virtual Machine Instruction Set </a>
 */
public final class Opcode {
	 ...
	 * 
	 * @param code
	 *            Byte array of method source code
	 * @param classFile
	 *            The {@link ClassFile} object contains the method
	 * @return Readable string of the method source code
	 */
	public static CodeResult parseCode(final byte[] code,
	...
	private static OpcodeResult parseOpcode(final PosDataInputStream pdis,
			final ClassFile classFile) throws IOException {
		...
		case Opcode.op_ldc:
			...
			// runtime constant pool of the current class (?.6).
			...
			opcodeText = String.format(FORMAT_SDS, Opcode.name_ldc, byteValue,
					classFile.getCPDescription(byteValue));
			break;
		...
		case Opcode.op_ldc_w:
			...
			// where the value of the index is calculated as (indexbyte1 << 8) |
			// indexbyte2.
			// The index must be a valid index into the runtime constant pool of
			// the current class.
			...
			opcodeText = String.format(FORMAT_SDS, Opcode.name_ldc_w,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_ldc2_w:
			...
			// The index must be a valid index into the runtime constant pool of
			// the current class.
			// The runtime constant pool entry at the index must be a runtime
			// constant of type long or double (?.1).
			// The numeric value of that runtime constant is pushed onto the
			// operand stack as a long or double, respectively.
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_ldc2_w,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_iload:
			...
			// local variable array of the current frame (?.6).
			// The local variable at index must contain an int.
			// The value of the local variable at index is pushed onto the
			// operand stack.
			byteValue = pdis.readUnsignedByte();
		...
		case Opcode.op_lload:
			...
			// Both index and index + 1 must be indices into the local variable
			// array of the current frame (?.6).
		...
		case Opcode.op_aload:
			...
			// The objectref in the local variable at index is pushed onto the
			// operand stack.
			byteValue = pdis.readUnsignedByte();
		...
		// Load int from local variable
		// The <n> must be an index into the local variable array of the current
		// frame (?.6).
		// The local variable at <n> must contain an int.
		// The value of the local variable at <n> is pushed onto the operand
		// stack.
		...
		// Load long from local variable
		// Both <n> and <n> + 1 must be indices into the local variable array of
		// the current frame (?.6).
		...
		// The int value in the component of the array at index is retrieved and
		...
		case Opcode.op_istore:
			...
			// It is popped from the operand stack, and the value of the local
			// variable at index is set to value.
			byteValue = pdis.readUnsignedByte();
			opcodeText = String
					.format(FORMAT_SD, Opcode.name_istore, byteValue);
			break;
		...
		case Opcode.op_lstore:
			...
			// It is popped from the operand stack, and the local variables at
			// index and index + 1 are set to value.
			byteValue = pdis.readUnsignedByte();
			opcodeText = String
					.format(FORMAT_SD, Opcode.name_lstore, byteValue);
			break;

		// Store float into local variable
		// The index is an unsigned byte that must be an index into the local
		// variable array of the current frame (?.6).
		...
		case Opcode.op_dstore:
			byteValue = pdis.readUnsignedByte();
			opcodeText = String
					.format(FORMAT_SD, Opcode.name_dstore, byteValue);
			break;

		// Store reference into local variable
		// The index is an unsigned byte that must be an index into the local
		// variable array of the current frame (?.6).
		// The objectref on the top of the operand stack must be of type
		// returnAddress or of type reference.
		// It is popped from the operand stack, and the value of the local
		...
		// the current frame (?.6).
		// The value on the top of the operand stack must be of type long.
		// It is popped from the operand stack, and the local variables at <n>
		// and <n> + 1 are set to value.
		...
		// returnAddress or of type reference.
		// It is popped from the operand stack, and the value of the local
		// variable at <n> is set to objectref.
		...
		// The int value is stored as the component of the array indexed by
		// index.
		...
		// category 1 computational type (?.11.1).
		case Opcode.op_pop:
		...
		// down
		// Form 1: ..., value3, value2, value1 --> ..., value1, value3, value2,
		// value1
		...
		// both values of a category 1 computational type (?.11.1).
		case Opcode.op_swap:
		...
		// if<cond>: ifeq = 153 (0x99) ifne = 154 (0x9a) iflt = 155 (0x9b) ifge
		// = 156 (0x9c) ifgt = 157 (0x9d) ifle = 158 (0x9e)
		// Branch if int comparison with zero succeeds
		//
		// If the comparison succeeds, the unsigned branchbyte1 and branchbyte2
		// are used to construct a signed 16-bit offset,
		// where the offset is calculated to be (branchbyte1 << 8) |
		// branchbyte2.
		// Execution then proceeds at that offset from the address of the opcode
		// of this if<cond> instruction.
		// The target address must be that of an opcode of an instruction within
		// the method that contains this if<cond> instruction.
		case Opcode.op_ifeq:
		...
		// if_icmp<cond>: if_icmpeq = 159 (0x9f) if_icmpne = 160 (0xa0)
		// if_icmplt = 161 (0xa1) if_icmpge = 162 (0xa2) if_icmpgt = 163 (0xa3)
		// if_icmple = 164 (0xa4)
		// Branch if int comparison succeeds
		case Opcode.op_if_icmpeq:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_icmpeq,
					shortValue);
			break;

		case Opcode.op_if_icmpne:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_icmpne,
					shortValue);
			break;

		case Opcode.op_if_icmplt:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_icmplt,
					shortValue);
			break;

		case Opcode.op_if_icmpge:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_icmpge,
					shortValue);
			break;

		case Opcode.op_if_icmpgt:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_icmpgt,
					shortValue);
			break;

		case Opcode.op_if_icmple:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_icmple,
					shortValue);
			break;
		...
		// popped from the operand stack and compared.
		//
		// If the comparison succeeds, the unsigned branchbyte1 and branchbyte2
		// are used to construct a signed 16-bit offset,
		// where the offset is calculated to be (branchbyte1 << 8) |
		// branchbyte2.
		// Execution then proceeds at that offset from the address of the opcode
		// of this if_acmp<cond> instruction.
		// The target address must be that of an opcode of an instruction within
		// the method that contains this if_acmp<cond> instruction.
		case Opcode.op_if_acmpeq:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_acmpeq,
					shortValue);
			break;

		case Opcode.op_if_acmpne:
			shortValue = pdis.readShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_if_acmpne,
					shortValue);
			break;

		// Branch always
		// The unsigned bytes branchbyte1 and branchbyte2 are used to construct
		// a signed 16-bit branchoffset,
		...
		// Execution proceeds at that offset from the address of this jsr
		// instruction.
		// The target address must be that of an opcode of an instruction within
		// the method that contains this jsr instruction.
		case Opcode.op_jsr:
			shortValue = pdis.readShort();
		...
		// The local variable at index in the current frame (?.6) must contain a
		// value of type returnAddress.
		// The contents of the local variable are written into the Java virtual
		// machine's pc register, and execution continues there.
		case Opcode.op_ret:
			byteValue = pdis.readUnsignedByte();
		...
		case Opcode.op_getstatic:
			...
			// index into the runtime constant pool of the current class (?.6),
			...
			// which gives the name and descriptor of the field as well as a
			// symbolic reference to the class or interface
			// in which the field is to be found.
			// The referenced field is resolved (?.4.3.2).
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_getstatic,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_putstatic:
			...
			// which gives the name and descriptor of the field as well as a
			// symbolic reference to the class or interface
			// in which the field is to be found. The referenced field is
			// resolved (?.4.3.2).
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_putstatic,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_getfield:
			...
			// reference to a field (?.1),
			// which gives the name and descriptor of the field as well as a
			// symbolic reference to the class in which the field is to be
			// found.
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_getfield,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_putfield:
			...
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_putfield,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_invokevirtual:
			...
			// where the value of the index is (indexbyte1 << 8) | indexbyte2.
			// The runtime constant pool item at that index must be a symbolic
			// reference to a method (?.1),
			// which gives the name and descriptor (?.3.3) of the method as well
			// as a symbolic reference to the class
			// in which the method is to be found.
			// The named method is resolved (?.4.3.3).
			// The method must not be an instance initialization method (?.9) or
			// the class or interface initialization method (?.9).
			...
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_invokevirtual,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_invokespecial:
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_invokespecial,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_invokestatic:
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_invokestatic,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_invokeinterface:
			...
			opcodeText = String.format("%s interface=%d [%s], nargs=%d",
					Opcode.name_invokeinterface, shortValue,
					classFile.getCPDescription(shortValue), byteValue);
			break;
		...
		case Opcode.op_new:
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_new, shortValue,
		...
		case Opcode.op_newarray:
			byteValue = pdis.readUnsignedByte();
			opcodeText = String.format("%s %s", Opcode.name_newarray,
		...
		case Opcode.op_anewarray:
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_anewarray,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_checkcast:
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_checkcast,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_instanceof:
			shortValue = pdis.readUnsignedShort();
			parseResult.CPIndex1 = shortValue;
			opcodeText = String.format(FORMAT_SDS, Opcode.name_instanceof,
					shortValue, classFile.getCPDescription(shortValue));
			break;
		...
		case Opcode.op_multianewarray:
			...
			opcodeText = String.format("%s index=%d [%s] dimensions=%d",
					Opcode.name_instanceof, shortValue,
					classFile.getCPDescription(shortValue), byteValue);
			break;
		...
		case Opcode.op_ifnull:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_ifnull,
					shortValue);
			break;
		...
		case Opcode.op_ifnonnull:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_ifnonnull,
					shortValue);
			break;
		...
		case Opcode.op_jsr_w:
			// The unsigned branchbyte1, branchbyte2, branchbyte3, and
			// branchbyte4 are used to
		...
		parseResult.OpcodeText = String.format("opcode [%02X] - %04d: %s",
				opcode, curPos, opcodeText);
		return parseResult;
	}
	...
	private static String getOpcodeLookupSwitchText(
			final PosDataInputStream pdis, final int spaceLength)
			throws IOException {
		int i;
		String space;
		if (spaceLength < 1) {
	...
	private static String getOpcodeTableSwitchText(
			final PosDataInputStream pdis, final int spaceLength)
			throws IOException {
		int i;
		String space;
		...
		sb.append(String.format(" %d to %d: default=%d", valueLow, valueHigh,
				defaultJump));
	...
	private static String getOpcodeWideText(final PosDataInputStream pdis)
		...
		case Opcode.op_iload:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_iload,
					shortValue);
			break;

		case Opcode.op_lload:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_lload,
					shortValue);
			break;

		case Opcode.op_fload:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_fload,
					shortValue);
			break;

		case Opcode.op_dload:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_dload,
					shortValue);
			break;

		case Opcode.op_aload:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_aload,
					shortValue);
			break;

		case Opcode.op_istore:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_istore,
					shortValue);
			break;

		case Opcode.op_lstore:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_lstore,
					shortValue);
			break;

		case Opcode.op_fstore:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_fstore,
					shortValue);
			break;

		case Opcode.op_dstore:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_dstore,
					shortValue);
			break;

		case Opcode.op_astore:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_astore,
					shortValue);
			break;
		...
		case Opcode.op_iinc:
			...
			opcodeText = String.format("%s %d %d", Opcode.name_wide_iinc,
					shortValue, shortValue2);
			break;

		case Opcode.op_ret:
			shortValue = pdis.readUnsignedShort();
			opcodeText = String.format(FORMAT_SD, Opcode.name_wide_ret,
					shortValue);
			break;
