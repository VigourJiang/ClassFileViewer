...
final class SignatureConvertor {
	...
	protected static String parseMethodReturnType(final String signature)
		...
		if (signature.length() < 3) {
			throw new IllegalArgumentException(
					"'signature' should be more than 2 characters.");
		}

		final int bracketEnd = signature.indexOf(')');
		if (bracketEnd == -1) {
			throw new IllegalArgumentException(String.format(
					"There is no ')' in the method signature: %s", signature));
		}
	...
	protected static String parseMethodParameters(final String signature)
		...
		if ((signature.charAt(0) != '(') || (bracketEnd == -1)) {
			throw new IllegalArgumentException(String.format(
					"There is no '(' or ')' in the method signature: %s",
					signature));
		}
		...
		final StringBuilder sbResult = new StringBuilder(signature.length()
				+ signature.length());
		sbResult.append('(');
		...
		while (rawParameters.charAt(0) != ')') {
			if (rawParameters.charAt(0) == '[') {
				...
				if (SignatureConvertor
						.isPrimitiveTypes(rawParameters.charAt(0))) {
					sbParameter = sbParameter.insert(0, SignatureConvertor
							.extractPrimitiveTypes(rawParameters.charAt(0)));
					...
					sbParameter = sbParameter.insert(0, SignatureConvertor
							.extractClassFullSignature(rawParameters.substring(
									0, commaIndex + 1)));
					sbResult.append(sbParameter);
					parametersCounter++;
					sbParameter = sbParameter.delete(0, sbParameter.capacity());
	...
	private static String rawSignature2Type(final String signature)
		...
		case 2:
			throw new SignatureException(
					String.format(
							"Sinagure length cannot be 0, or 2. Current 'signature' length=%d.",
							signatureLength));
	...
	private static String extractClassFullSignature(final String classSignature)
		...
		if (classSignature == null) {
			throw new IllegalArgumentException(
					"'ClassSignature' should not be null.");
		}
		...
		if (length < 3) {
			throw new SignatureException(
					String.format(
							"Fully-qualified class sinagure length cannot be less than 2. Current length=%d.",
							length));
		}
		if ((classSignature.charAt(0) != 'L')
				|| (classSignature.charAt(length - 1) != ';')) {
			throw new SignatureException(
					String.format(
							"Fully-qualified class sinagure format is not 'L-xxx-;'. it is '%s'.",
							classSignature));
		}
